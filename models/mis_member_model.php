...
class Mis_member_model extends NF_Model {
    const TABLE = 'member';
    ...
    //查詢資料(分頁)
    public function get_memberpage($query_info,$page) {
    ...
    //新增資料(單筆)
    public function  add_member($data,$key_field=False){
        return $this->add($data ,$key_field);
    }
	//查詢資料
    public function get_member($key_field) {
        $param = new ModelParameter();
        $param->select('*');
	 ...
	 * 更新資料:可以更新key
    */
    public function  upd_member_ci($data,$where){
    ...
    //更新資料
    public function  upd_member($data,$key_field){
        return $this->save($data,$key_field);
    }
    //刪除資料
    public function  del_member($key_field){    	
        return $this->del($key_field);
    }
	//查詢資料
    public function get_member_all($key_field) {
    	$param = new ModelParameter();
        $param->select('*');
        $param->where($key_field);
    ...
    //查詢單一欄位資料，回傳array
    public function  get_member_col($col){
        $param = new ModelParameter();
        $param->select($col);
