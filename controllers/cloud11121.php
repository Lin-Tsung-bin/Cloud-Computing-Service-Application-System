...
class Cloud11121 extends NF_Controller {
	// 1.基礎設定
	private $code_name = 'cloud11121'; // 程式編號
	...
	*/
	private $show_field = array (
		   ...
		   'apply_deptname' => array (
					'chi_name' => '申請人單位名稱',	
					'grid'     => array('width'=>130),						
					'form'     => array('editable'=>'11'),   //20180212
					'query'    => array('textcase'=> 'toUpperCase',
									    'defaultval'=>array()
								 ),
					'validate'=>'required',
...
$pdf->MultiCell($pdf_width, $pdf_cell_height, '電話', 1, 'C', 0, 0, '','', false, '',true, false, $pdf_cell_height, 'M');$pdf->MultiCell($pdf_width, $pdf_cell_height, $data->apply_phone, 1, 'C', 0, 0, '', '', false, '', true, false, $pdf_cell_height, 'M');
$pdf->Ln();
...
$pdf->Ln();
	...
	public function machine_open_mail() {
		//ob_start();
		...
		$out = array('success' => 'F', 'msg' => '');
		$field=array('receiver','title','content1');
		$data=$this->input->post_parameters($field);
		$data['content1']=trim($data['content1']);
		$data['content1'] = preg_replace('/\s(?=\s)/', '',$data['content1']);
		$data['content1'] = preg_replace('/[\n\r\t]/', ' ',$data['content1']);	 	
		$data['content1'] =str_replace("@@@@@","&", $data['content1'] );
		$data['sender'] = $this->config->item('machine_sender');  //從config.php 找取machine_sender mail
		 ...
		 if (!$data['title']) {
			...
			$this->email->message($data['content1']);
			

			$send_rtn=$this->email->send();
			//狀態更新
			$upd_data = array('apply_id'=>$apply_id,'status'=>'2');
			$rtn=$this->$model_setting ['model']->$model_setting ['upd_one'] ($upd_data,$model_setting['key_field']);
			//echo $this->db->last_query();
			if($send_rtn){
				$out['success'] = 'T';	
				$out['msg'] = '發送成功!';	   
				//$out['msg'] = '已送審!';		//value='2', 已開通; value='3', 已關閉
	...
	public function send1(){
    	...
    	$this->email->set_mailtype("html");			
		$this->email->clear();
		$this->email->from($config=$this->config->$config['machine_sender']);   //管理者mail
		//$data['apply_email'] = array('daffodil0925@gmail.com', '10163128@gm.nfu.edu.tw');  //2180330不能使用
		$this->email->to($data['apply_email']);  //使用者mail $data['apply_email'];
		$this->email->subject("雲端計算服務開通通知");			
		$this->email->message("");
	...
	public function money_paid_mail() {
		//ob_start();
		...
		$out = array('success' => 'F', 'msg' => '');
		$field=array('receiver2','title2','content2');
		$data=$this->input->post_parameters($field);
		$data['content2']=trim($data['content2']);
		$data['content2'] = preg_replace('/\s(?=\s)/', '',$data['content2']);
		$data['content2'] = preg_replace('/[\n\r\t]/', ' ',$data['content2']);	 	
		$data['content2'] =str_replace("@@@@@","&", $data['content2'] );
		$data['sender2'] = $this->config->item('money_sender');  //從config.php 找取machine_sender mail
		
		 if (!$data['title2']) {
			$out['msg'] = "未填寫主旨";
			//3. 檢查內文
		} else if (!$data['content2']) {
			$out['msg'] = "未填寫內文";
			//4. 檢查單位
		} else if (!$data['receiver2']) {
			$out['msg'] = "未填寫收件者";
			//4. 檢查單位
		}else if (!$data['sender2']) {
			$out['msg'] = "未填寫寄件者";
			//4. 檢查單位
		} else {
	    	...
	    	$this->email->set_mailtype("html");		
			$this->email->clear(TRUE);	
			$this->email->from($data['sender2']);
			$this->email->to($data['receiver2']);
			$this->email->subject($data['title2']);	
			$this->email->message($data['content2']);
	...
	public function send2(){
    	...
    	$this->email->set_mailtype("html");			
		$this->email->clear();	
		$this->email->from($config=$this->config->$config['money_sender']); //管理者mail  $data['user_mail']
		$this->email->to($data['apply_email']);  //使用者mail $data['apply_email']
		$this->email->subject("雲端計算服務繳費通知");			
		$this->email->message("");
