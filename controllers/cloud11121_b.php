...
*
*/
class Cloud11121_b extends NF_Controller {
	// 1.基礎設定
	private $code_name = 'cloud11121_b'; // 程式編號
	...
	*/
	private $show_field = array (
		   ...
		   'os' => array (
					'chi_name' => '作業系統',	
					'grid'     => array('width'=>100),						
					'form'     => array('editable'=>'11','initial'=> 'W'), 
		   ...
		   'receipt' => array (
			...
			'is_paid' => array (
					...
					'form'     => array('editable'=>'11','initial'=> 'D'), 
					'query'    => array('queryType'=> 'dropdownlist',
    ...
    public function add()
    {
    	if($_POST){
			...
			if (!empty($validate_rtn)) {
				...
				//20180202 增加
				$this->Auth_service->get_user_name();
				//$data['register_psncode'] = isset($_POST['register_psncode']) ? trim($_POST['update_psncode']) : trim($_POST['update_psncode']);   //20180204
				//$data['register_datetime'] = isset($_POST['register_datetime']) ? trim($_POST['update_datetime']) : trim($_POST['update_datetime']);  //20180204
				//$data['register_ip'] = isset($_POST['register_ip']) ? trim($_POST['update_ip']) : trim($_POST['update_ip']);  //20180204
				$data['register_psncode']=$data['update_psncode'];     //20180202
				$data['register_datetime']=$data['update_datetime'];   //20180202
				$data['register_ip']=$data['update_ip'];				 //20180202
				//$data['register_psnname']=$data['name'];
				//$data['register_deptcode']=$data[''];
				//$data['register_deptname']=$data['dept_name'];
				...
				if(!$serial_data){
					...
					// $data['apply_id'] = $this_mon.str_pad($new_serial,5,"0",STR_PAD_LEFT);  --> 20180201檢查出錯誤:產生107021070200002錯誤，超過10個字串;正確1070200002，要刪除'$this_mon.'
												...
												//pad_string:可選。規定供填充使用的字串。默認是空白。
												//pad_type: 可選。規定填充字串的哪邊。 STR_PAD_RIGHT - 填充字串的右側。
					}
    ...
    public function export_pdf() {   //20180119 - 1084至1134 另外弄成一個project
		...
		*/		
    	...
    	foreach($data as $index=>$row){
    		...
    		$pdf->Ln();   // 20180201建立pdf表單要空格 格式: $pdf->MultiCell(); $pdf->Ln();  $pdf->MultiCell();  -->...-->
			//加上資料序號
